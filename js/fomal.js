...
function percent() {
  ...
  let icon = btn.querySelector('i.fas.fa-arrow-up');
  let percentEl = btn.querySelector('#percent');

  if (result < 95) { // 如果阅读进度小于95% 就显示百分比
    if (icon) icon.style.display = 'none';
    if (percentEl) {
      percentEl.style.display = 'block';
      percentEl.innerHTML = result + '<span>%</span>';
    }
...
function showWelcome() {
  // 防御：ipLoacation可能未初始化，避免脚本中断
  if (!window.ipLoacation || !ipLoacation.result || !ipLoacation.result.location || !ipLoacation.result.ad_info) {
    return;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  if (localStorage.getItem("blogbg") != undefined) {
    ...
    document.getElementById("defineBg").innerText = `:root{
      --default-bg: url(https://potatojiang.oss-cn-chengdu.aliyuncs.com/background-day.png);
      --darkmode-bg:url(https://potatojiang.oss-cn-chengdu.aliyuncs.com/background-night.jpg);
      --mobileday-bg: url(https://potatojiang.oss-cn-chengdu.aliyuncs.com/mobel-day.jpg);
      --mobilenight-bg: url(https://potatojiang.oss-cn-chengdu.aliyuncs.com/mobel-night.png);
    }`;
  }
});
// 切换背景主函数
