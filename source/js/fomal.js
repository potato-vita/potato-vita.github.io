...
function tonav() {
  ...
  const menuList = document.getElementsByClassName("menus_items");
  const primaryMenu = menuList.length > 0 ? menuList[0] : null;

  if (!nameContainer || !pageNameEl) return;
  ...
  $(window).on("scroll.tonav", function () {
    ...
    if (scroll > position) {
      ...
      if (primaryMenu) primaryMenu.setAttribute("style", "display:none!important");
    } else {
      if (primaryMenu) primaryMenu.setAttribute("style", "");
      nameContainer.setAttribute("style", "display:none");
    }
    position = scroll;
  });

  const titleText = document.title.includes(" | ") ? document.title.split(" | ")[0] : document.title;
  pageNameEl.innerText = titleText;
}
